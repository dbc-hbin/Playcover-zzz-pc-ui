...
static int pt_open(char const* restrict filename, int oflag, ... ) {
    ...
    if ((oflag & O_CREAT) != 0) {
        ...
        return openat(AT_FDCWD, filename, oflag, mode);
    }

    return openat(AT_FDCWD, filename, oflag);
}

static int pt_stat(char const* restrict path, struct stat* restrict buf) {
    return fstatat(AT_FDCWD, ue_fix_filename(path), buf, 0);
}

static int pt_access(char const* path, int mode) {
    return faccessat(AT_FDCWD, ue_fix_filename(path), mode, 0);
}

static int pt_rename(char const* restrict old_name, char const* restrict new_name) {
    return renameat(
        AT_FDCWD,
        ue_fix_filename(old_name),
        AT_FDCWD,
        ue_fix_filename(new_name)
    );
}

static int pt_unlink(char const* path) {
    return unlinkat(AT_FDCWD, ue_fix_filename(path), 0);
}

static int pt_nanosleep(useconds_t time) {
    struct timespec request = {
        .tv_sec = time / 1000000,
        .tv_nsec = (long)(time % 1000000) * 1000,
    };
