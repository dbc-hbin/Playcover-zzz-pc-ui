...
extension UIApplication {
    ...
    @objc
    func toggleExperimentalUnityNativeMouse(_ sender: AnyObject) {
        settings.experimentalUnityNativeMouse.toggle()
        UIMenuSystem.main.setNeedsRebuild()
        let state = settings.experimentalUnityNativeMouse ? "enabled" : "disabled"
        Toast.showHint(title: "Unity native mouse \(state); restart the game")
    }
...
class MenuController {
    ...
    func setupMenu(with builder: UIMenuBuilder) {
        ...
        builder.insertSibling(MenuController.inputCompatibilityMenu(), afterMenu: .view)
    }
    ...
    static func inputCompatibilityMenu() -> UIMenu {
        let command = UICommand(
            title: "Unity Native Mouse (Experimental)",
            image: UIImage(systemName: "computermouse"),
            action: #selector(UIApplication.toggleExperimentalUnityNativeMouse(_:)),
            propertyList: nil
        )
        command.state = settings.experimentalUnityNativeMouse ? .on : .off
        return UIMenu(
            title: "Input Compatibility",
            image: nil,
            identifier: .inputCompatibilityMenu,
            options: [],
            children: [command]
        )
    }
...
extension UIMenu.Identifier {
    ...
    static var inputCompatibilityMenu: UIMenu.Identifier {
        UIMenu.Identifier("io.playcover.PlayTools.menus.input-compatibility")
    }
}
